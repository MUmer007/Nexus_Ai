...
def parse_request(state: AgentState) -> AgentState:
    ...
    if "reallocate" in request and "inventory" in request:
        ...
        state["action_details"] = {"recipient": "ops-team", "message": "Inventory alert"}
        state["action_id"] = str(uuid.uuid4())
        print(f"   ➔ Action: {state['action_type']}")
        print(f"   ➔ Action ID: {state['action_id']}")
    else:
...
def execute_action(state: AgentState) -> AgentState:
    ...
    if state["action_type"] == "reallocate_inventory":
        result = reallocate_inventory(
            state["action_details"]["region"],
            state["action_details"]["percentage"]
        )
    elif state["action_type"] == "send_notification":
        result = send_notification(
            state["action_details"]["recipient"],
            state["action_details"]["message"]
        )
    else:
...
workflow.add_conditional_edges(
    "evaluate_policy", should_continue,
    {"wait_for_approval": "wait_for_approval", "execute": "execute_action", "reject": "handle_rejection"}
)
workflow.add_conditional_edges(
    "wait_for_approval", check_human_decision,
...
if __name__ == "__main__":
    ...
    result1 = agent.invoke({
        ...
        "action_type": None, "action_details": None, "action_id": None,
        "policy_decision": None, "human_decision": None, "execution_result": None
    })
