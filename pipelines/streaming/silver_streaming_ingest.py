...
from pyspark.sql.functions import col, from_json, current_timestamp, upper
from pyspark.sql.types import StructType, StructField, IntegerType, StringType, LongType

def main():
    spark = SparkSession.builder \
        .appName("NEXUS_Silver_Streaming_Ingestion") \
    ...
    payload_schema = StructType([
        ...
        StructField("__deleted", StringType(), True)       # 'true' or 'false' (replaces __op)
    ])
    ...
    silver_df = parsed_df.select(
        ...
        (col("envelope.payload.order_date") / 1000000.0).cast("timestamp").alias("order_timestamp"),
