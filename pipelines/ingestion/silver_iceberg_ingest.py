...
from pyspark.sql.functions import col, current_timestamp, to_timestamp, upper, when
...
def main():
    ...
    spark = (
        SparkSession.builder.appName("NEXUS_Silver_Iceberg_Ingestion")
    ...
    df_silver = (
        ...
        .withColumn(
            "is_cancelled", when(col("status") == "CANCELLED", True).otherwise(False)
        )
    ...
    df_silver.writeTo("local.nexus.silver_orders").tableProperty(
        "format-version", "2"
    ).createOrReplace()
