...
def load_model():
    ...
    print(f"✅ Model loaded successfully from run: {latest_run_id} (Accuracy: {runs.iloc[0]['metrics.accuracy']:.4f})")
...
def predict(features: OrderFeatures):
    ...
    df['is_night_order'] = 1 if df['hour_of_day'].iloc[0] >= 22 or df['hour_of_day'].iloc[0] <= 5 else 0
    df['is_high_risk_combo'] = 1 if (df['is_high_value'].iloc[0] == 1 and df['is_night_order'].iloc[0] == 1) else 0
