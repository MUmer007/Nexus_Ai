...
customer_df = pd.DataFrame({
    ...
    "tier": np.random.choice(["premium", "standard", "basic"], n_customers, p=[0.2, 0.5, 0.3]),
...
order_df = pd.DataFrame({
    ...
    "is_night_order": ((np.random.randint(0, 24, n_orders) >= 22) | (np.random.randint(0, 24, n_orders) <= 5)).astype(int),
